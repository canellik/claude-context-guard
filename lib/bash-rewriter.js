...
const RULES = [
  ...
  {
    id: 'tree-unbounded',
    test: (cmd) => /(?:^|[\s;|&(])tree(?![-\w])/.test(cmd) && !/-L\s+\d+/.test(cmd) && !/--noreport/.test(cmd),
  ...
  {
    id: 'cat-log',
    test: (cmd) => /(?:^|[\s;|&(])cat\s+[^|;&<]*\.(log|csv|tsv|jsonl|ndjson)\b/.test(cmd),
  ...
  {
    id: 'cat-lockfile',
    test: (cmd) => /(?:^|[\s;|&(])cat\s+[^|;&<]*(package-lock\.json|yarn\.lock|pnpm-lock\.yaml|Cargo\.lock|composer\.lock|Gemfile\.lock|poetry\.lock)\b/.test(cmd),
